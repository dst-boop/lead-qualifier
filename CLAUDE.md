...
- **Enrichment profiles are configurable.** Define named enrichment profiles (e.g., "basic verify," "full demographic," "outreach-ready") as config specifying which sources run and in what order. The operator picks the profile per campaign. *(Shipped: **Autopilot** — presets plus saved profiles in `settings.autopilot`; each step has tiers and a cap; one approval per run states the worst case; the run record and the WhitePages cap live server-side in `/api/autopilot`, enforced in `_wp_fetch` via `X-Autopilot-Job`. Outreach is never automated.)*
...
- **The master list is the Life Data record and outcomes close the loop.** "All leads" holds every lead ever landed — current, updated, former, future — and statuses (Set / Not Interested / Has Advisor / …) are outcome data, as much a part of the profile as any enriched field. The "What's converting" panel reads outcomes back against tiers, signals, employers, and enrichment spend, and the "Life Data export" emits the lifetime CSV. Analysis output is **suggestions, never silent retuning** — targeting weights live in ICP settings and only the operator changes them. Rates are shown only above minimum sample sizes; a percentage computed on four calls is noise.
